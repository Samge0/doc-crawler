...
# -*- coding: utf-8 -*-
# author：samge
# date：2024-08-23 13:48
# describe：

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time    : 2022/4/19 下午3:15
# @Author  : Samge
import random
...
def random_chrome() -> str:
    while True:
        value = get_r_user_agent().pc()
        if 'Chrome' in value:
            return value


def random_firefox() -> str:
    while True:
        value = get_r_user_agent().pc()
        if 'Firefox' in value:
            return value
...
if __name__ == '__main__':
    print(f"\nlocal       random：{random_one()}")

    u = UserAgent()
    print(f"r-useragent random：{u.random()}")
    print(f"r-useragent random：{u.pc()}")
